...
import com.hostpital.hospitaladmin.Adapters.UserRequestAdapter;
...
/**
 * Created by Arbab on 8/15/2019.
 */

public class ShowUserRequestsFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public ShowUserRequestsFragment() {
    }

    public static ShowUserRequestsFragment newInstance() {
        ShowUserRequestsFragment fragment = new ShowUserRequestsFragment();
        return fragment;
    }
...
//        ((AppCompatActivity) getActivity()).getSupportActionBar().setTitle("Show User Request");
...
//        checkUserToken();
        ...
        adapter = new UserRequestAdapter(getActivity(), userRequestsListList);
...
//                        SharedPreferenceUtil.storeStringValue(context, "request_status", jsonObject.optString("status"));
                                ...
                                userRequests.setAddress(jsonObject.getString("address"));
                                userRequests.setStart_time(jsonObject.getString("start_time"));
                                userRequests.setEnd_time(jsonObject.getString("end_time"));


                                JSONObject acceptObject = jsonObject.getJSONObject("accept_request");

                                JSONObject doctorObject = acceptObject.getJSONObject("doctor");
                                userRequests.setRequest_admin_name(doctorObject.getString("name"));
                                userRequests.setRequest_admin_phone(doctorObject.getString("phone"));

                                JSONObject employObject = doctorObject.getJSONObject("employ");
                                userRequests.setJob_title(employObject.getString("job_title"));
                                userRequests.setGraduation_date(employObject.getString("graduation_date"));
                                userRequests.setUser_address(employObject.getString("address"));
                                userRequests.setYears_of_experience(employObject.getString("years_of_experience"));
                                userRequests.setCv(employObject.getString("cv"));

                                JSONObject medicalBoardObject = employObject.getJSONObject("medical_board");
                                userRequests.setRegistration_number(medicalBoardObject.getString("registration_number"));
                                userRequests.setField(medicalBoardObject.getString("field"));
